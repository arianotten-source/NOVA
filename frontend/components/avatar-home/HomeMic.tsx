...
export default function HomeMic() {
  ...
  const { voiceState, micSupported, toggleListening, wakeWordListening } = useVoicePipeline();
  ...
  const isWaiting = voiceState === VoiceState.WAITING;
      ...
      <button
        ...
        className={cn(
          ...
          isListening || isWaiting
            ? 'border-nova-cyan shadow-[0_0_28px_rgba(0,255,245,0.3)] animate-pulse'
            : isThinking
              ? 'border-amber-400/60 shadow-[0_0_20px_rgba(251,191,36,0.2)]'
              : wakeWordListening
                ? 'border-nova-cyan/40 shadow-[0_0_12px_rgba(0,255,245,0.15)]'
      ...
      >
        {disabled ? (
          ...
          <Mic
            className={cn(
              'w-6 h-6',
              isListening || isWaiting ? 'text-nova-cyan' : isThinking ? 'text-amber-300' : ''
            )}
          />
        )}
      </button>
